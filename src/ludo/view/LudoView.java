...
public class LudoView {
	...
	public void printBoard(LudoModel model) {
		...
		for(int i = 0; i<4; i++){
			...
			} else if(model.getPlayerList()[Player.RED].getGamePiece(i).getPosition() != GamePiece.IN_HOME){
				tempPosition = POSITION_MAPPINGS[model.getPlayerList()[Player.RED].getGamePiece(i).getPosition()]; 
			}
			tempBoard[tempPosition.getX()][tempPosition.getY()] = "R"+i;
		}
		
		for(int i = 0; i<4; i++){
			if(model.getPlayerList()[Player.YELLOW].getGamePiece(i).getPosition() == GamePiece.IN_START){
				tempPosition = START_POSITIONS[Player.YELLOW][i];
			} else if(model.getPlayerList()[Player.YELLOW].getGamePiece(i).getPosition() != GamePiece.IN_HOME){
				tempPosition = POSITION_MAPPINGS[model.getPlayerList()[Player.YELLOW].getGamePiece(i).getPosition()]; 
			} 
			tempBoard[tempPosition.getX()][tempPosition.getY()] = "Y"+i;
		}
		...
		for(int i = 0; i<4; i++){
			...
			} else if(model.getPlayerList()[Player.GREEN].getGamePiece(i).getPosition() != GamePiece.IN_HOME){
				tempPosition = POSITION_MAPPINGS[model.getPlayerList()[Player.GREEN].getGamePiece(i).getPosition()]; 
			} 
			tempBoard[tempPosition.getX()][tempPosition.getY()] = "G"+i;
		}
		...
		for(int i = 0; i<4; i++){
			...
			} else if(model.getPlayerList()[Player.BLUE].getGamePiece(i).getPosition() != GamePiece.IN_HOME){
				tempPosition = POSITION_MAPPINGS[model.getPlayerList()[Player.BLUE].getGamePiece(i).getPosition()]; 
			}
			tempBoard[tempPosition.getX()][tempPosition.getY()] = "B"+i;
		}
